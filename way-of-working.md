...
4. After approval, the job writes `.uplink/reports/<id>/approval.md` (receipt pointing at the run), then `git uplink approve` and `git uplink submit` (contrib push). The same step opens the public PR with the upstream credential (`POST /repos/{parent}/pulls`, `maintainer_can_modify` false). Contrib App credentials exist only now. Submit is still the first time contribution bytes leave the private forge.
