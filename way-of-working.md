...
Create a repository Environment named **`to-upstream`**. Required reviewers are IP/legal. Put the GitHub App secrets that can push the public contribution fork **on that environment only**. The public PR is opened with the repository upstream credential (`POST /repos/{parent}/pulls`, `maintainer_can_modify` false, `head` is the branch and `head_repo` is the contrib repository name), which has contents read on the parent and the fork and pull requests write, and no contents write on the fork. That App or fine-grained PAT works when both repositories are in the same organization. A fork under a different organization or a user needs a machine user with a classic PAT. GitHub will not put both under one user account. Dispatch `Uplink submit` with the patch id. The workflow:
...
4. After approval, the job writes `.uplink/reports/<id>/approval.md` (receipt pointing at the run), then `git uplink approve` and `git uplink submit` (contrib push). The same step opens the public PR with the upstream credential (`POST /repos/{parent}/pulls`, `maintainer_can_modify` false, `head` is the branch and `head_repo` is the contrib repository name). Contrib App credentials exist only now. Submit is still the first time contribution bytes leave the private forge.
...
`git uplink transfer <id> --to-upstream` or `--to-internal` moves a patch between `internal[]` and `upstream[]`. The patch must already be in the source queue. `--to-upstream` re-assesses with intent `upstream` (replacing the skipped leak scan from internal import). If git apply, assess, **and** preflight (`UPLINK_PREFLIGHT` / export checks for `--to-upstream`) all pass, the move is committed immediately. If any fails, git-uplink cuts `uplink/transfer-to-*/<id>` plus `-work` and does **not** write `queue.json`. Merge the gated PR to complete; **close the PR without merging** to abort (branches deleted, queue unchanged). `--to-internal` refuses while another active patch still on `upstream[]` lists this id in `dependsOn` — transfer those dependents `--to-internal` first (internal is always last in the apply sequence, so `--to-upstream` does not need this). `--to-internal` of a submitted patch dispatches **Uplink abandon contrib**, which waits on Environment `abandon-contrib` to close the public PR and delete `uplink/<id>` on the contrib fork. Company `main` / `uplink/state` are already internal-only; the public leftover stays until that environment is approved.
