...
5. **Credentials** — three isolated roles. Workflows use a PAT (`UPLINK_*_TOKEN`) or mint an App installation token (`UPLINK_*_AUTH=app`). `POST /repos/{parent}/pulls` (`head` is the branch, `head_repo` is the contrib repository name, `maintainer_can_modify` false) uses the upstream TOKEN (repository secret). The contrib TOKEN only force-pushes the fork. Owner and repository name are `upstreamUrl` and `contribUrl` in `.uplink/queue.json` on `uplink/state`, not secrets.
...
| `UPLINK_UPSTREAM_TOKEN` | Authenticated `git fetch` of public upstream, and `GH_TOKEN` for `POST /repos/{parent}/pulls` (`head` is the branch, `head_repo` is the contrib repository name, `maintainer_can_modify` false) / `gh pr close`, when `UPLINK_UPSTREAM_AUTH=pat`. Contents read and pull requests write. Same organization: a fine-grained PAT that includes the parent and the fork. Different owner: a machine user classic PAT. No contents write on the fork |
...
approval.md committed; git uplink approve; git uplink submit
        │
        ▼
POST /repos/{parent}/pulls with head the branch, head_repo the contrib repository name, and maintainer_can_modify false (or reuse existing URL); git uplink submitted
        │
        ▼
...
gh api --method POST /repos/<parent>/pulls -f head='<branch>' -f head_repo='<contrib-repo>' -F maintainer_can_modify=false …
